 ...
 */ 
 ...
 .CSEG
	...
	jmp		Ext_Int5	; stop
	.ORG 0x000E
	jmp		Ext_Int6	;mpu data read + pwm calculation
	.ORG 0x0014
	;jmp		Timer2_ovf
	.ORG 0x001C
	;jmp		Timer1_ovf
...
;==============================
	...
	rcall	TWI_Write				; 8khz / 1+124  = 64 hz  [0x7c]						>>	1khz / 1+14  = 66 hz [0x0e]
...
CONV_DEG_Y:    ; 가속도 r5:r4 를 각도로 변환
	...
	;음수판별
    sbrc    r5, 7       ; r5의 7번째 비트(부호 비트)가 0이면 양수 → skip
    rjmp    MY_Y ; 음수 → 2의 보수화 루틴으로 점프
	;PUTC	'+'
    rjmp    PYY          ; 양수면 아무 작업도 하지 않고 종료
...
SPIN_CAL:
	...
	ori		temp, 0x01
    rjmp    SPIN_CAL1          ; 양수면 정방향 지정 후 점프
...
SPIN_CAL1:
    sbrc    r19, 7       ; r5의 7번째 비트(부호 비트)가 0이면 양수 → skip
    rjmp    RE_SPIN1			; 음수 → 2의 보수화 루틴으로 점프 후 역방향 지정
